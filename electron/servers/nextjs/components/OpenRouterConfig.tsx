"use client";
import { useState, useEffect } from "react";
import { Check, ChevronsUpDown, Loader2 } from "lucide-react";
import { Button } from "./ui/button";
import {
  Command,
  CommandEmpty,
  CommandGroup,
  CommandInput,
  CommandItem,
  CommandList,
} from "./ui/command";
import { Popover, PopoverContent, PopoverTrigger } from "./ui/popover";
import { cn } from "@/lib/utils";
import { toast } from "sonner";
import { getApiUrl } from "@/utils/api";

interface OpenRouterConfigProps {
  openrouterApiKey: string;
  openrouterModel: string;
  onInputChange: (value: string | boolean, field: string) => void;
}

export default function OpenRouterConfig({
  openrouterApiKey,
  openrouterModel,
  onInputChange,
}: OpenRouterConfigProps) {
  const [models, setModels] = useState<string[]>([]);
  const [modelsLoading, setModelsLoading] = useState(false);
  const [modelsChecked, setModelsChecked] = useState(false);
  const [openModelSelect, setOpenModelSelect] = useState(false);
  const [apiKey, setApiKey] = useState(openrouterApiKey);

  useEffect(() => {
    setModels([]);
    setModelsChecked(false);
    onInputChange("", "openrouter_model");
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [apiKey]);

  const onApiKeyChange = (value: string) => {
    setApiKey(value);
    onInputChange(value, "openrouter_api_key");
  };

  const fetchModels = async () => {
    if (!openrouterApiKey) return;

    try {
      setModelsLoading(true);
      const response = await fetch(getApiUrl("/api/v1/ppt/openrouter/models/available"), {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          api_key: openrouterApiKey,
        }),
      });

      if (response.ok) {
        const data = await response.json();
        setModels(data);
        setModelsChecked(true);
      } else {
        console.error("Failed to fetch OpenRouter models");
        setModels([]);
        setModelsChecked(true);
        toast.error("Failed to fetch OpenRouter models");
      }
    } catch (error) {
      console.error("Error fetching OpenRouter models:", error);
      toast.error("Error fetching OpenRouter models");
      setModels([]);
      setModelsChecked(true);
    } finally {
      setModelsLoading(false);
    }
  };

  return (
    <div className="space-y-6">
      {/* API Key Input */}
      <div className="mb-4">
        <label className="block text-sm font-medium text-gray-700 mb-2">
          OpenRouter API Key
        </label>
        <div className="relative">
          <input
            type="text"
            required
            placeholder="Enter your OpenRouter API Key"
            className="w-full px-4 py-2.5 outline-none border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors"
            value={openrouterApiKey}
            onChange={(e) => onApiKeyChange(e.target.value)}
          />
        </div>
        <p className="mt-1 text-xs text-gray-500">
          Get your API key at{" "}
          <a
            href="https://openrouter.ai/keys"
            target="_blank"
            rel="noopener noreferrer"
            className="text-blue-600 hover:underline"
          >
            openrouter.ai/keys
          </a>
        </p>
      </div>

      {/* Fetch Models Button */}
      {(!modelsChecked || (modelsChecked && models.length === 0)) && (
        <div className="mb-4">
          <button
            onClick={fetchModels}
            disabled={modelsLoading || !openrouterApiKey}
            className={`w-full py-2.5 px-4 rounded-lg transition-all duration-200 border-2 ${
              modelsLoading || !openrouterApiKey
                ? "bg-gray-100 border-gray-300 cursor-not-allowed text-gray-500"
                : "bg-white border-blue-600 text-blue-600 hover:bg-blue-50 focus:ring-2 focus:ring-blue-500/20"
            }`}
          >
            {modelsLoading ? (
              <div className="flex items-center justify-center gap-2">
                <Loader2 className="w-4 h-4 animate-spin" />
                Fetching models...
              </div>
            ) : (
              "Fetch available models"
            )}
          </button>
        </div>
      )}

      {/* No models found message */}
      {modelsChecked && models.length === 0 && (
        <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
          <p className="text-sm text-yellow-800">
            No models found. Please make sure your API key is valid.
          </p>
        </div>
      )}

      {/* Model selection dropdown */}
      {modelsChecked && models.length > 0 && (
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Select Model
          </label>
          <div className="w-full">
            <Popover open={openModelSelect} onOpenChange={setOpenModelSelect}>
              <PopoverTrigger asChild>
                <Button
                  variant="outline"
                  role="combobox"
                  aria-expanded={openModelSelect}
                  className="w-full h-12 px-4 py-4 outline-none border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors hover:border-gray-400 justify-between"
                >
                  <span className="text-sm font-medium text-gray-900">
                    {openrouterModel || "Select a model"}
                  </span>
                  <ChevronsUpDown className="w-4 h-4 text-gray-500" />
                </Button>
              </PopoverTrigger>
              <PopoverContent
                className="p-0"
                align="start"
                style={{ width: "var(--radix-popover-trigger-width)" }}
              >
                <Command>
                  <CommandInput placeholder="Search model..." />
                  <CommandList>
                    <CommandEmpty>No model found.</CommandEmpty>
                    <CommandGroup>
                      {models.map((model, index) => (
                        <CommandItem
                          key={index}
                          value={model}
                          onSelect={(value) => {
                            onInputChange(value, "openrouter_model");
                            setOpenModelSelect(false);
                          }}
                        >
                          <Check
                            className={cn(
                              "mr-2 h-4 w-4",
                              openrouterModel === model
                                ? "opacity-100"
                                : "opacity-0"
                            )}
                          />
                          <span className="text-sm font-medium text-gray-900">
                            {model}
                          </span>
                        </CommandItem>
                      ))}
                    </CommandGroup>
                  </CommandList>
                </Command>
              </PopoverContent>
            </Popover>
          </div>
        </div>
      )}

      {/* Info note */}
      <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
        <p className="text-sm text-blue-800">
          OpenRouter provides access to 100+ AI models. Tool Calls are used
          automatically for structured output, ensuring compatibility with all
          supported models.
        </p>
      </div>
    </div>
  );
}
